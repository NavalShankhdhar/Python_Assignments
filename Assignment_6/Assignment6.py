...
class MysqlConnection(MySQL):
    def create_table(self):
        self.cursor.execute("CREATE TABLE IF NOT EXISTS TWITTER_INFO(User_ID VARCHAR(255) , User_name VARCHAR(20) NOT NULL, TWEET VARCHAR(255), Date VARCHAR(25), Time VARCHAR(25), Favorites INT, Retweets INT, Source VARCHAR(50), URL VARCHAR(255))")
        self.connect.commit()
    def insert_row(self, tweet):
        self.connect = self.connect_database()
        self.cursor = self.connect.cursor()
        self.cursor.execute('INSERT INTO TWITTER_INFO(User_ID, User_name, TWEET, Date, Time, Favorites, Retweets, Source, URL) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s)', tuple(tweet))
        self.connect.commit()
        self.cursor.close()
...
class TwitterAuth:
    ...
    def extract_tweet_pdf(self, tweets):
        tweet_list = []
        for tweet in tweets:
            tweet_info =  [tweet.user.screen_name,(re.sub(r'https\S+', '', tweet.text)), tweet.favorite_count, tweet.retweet_count, re.findall('(?:(?:https?):\/\/)?[\w/\-?=%.]+\.[\w/\-&?=%.]+', tweet.text)]
            tweet_list.append(tweet_info)
        return tweet_list
